...
contract IoTDeviceOracle {
    ...
    function toggleDeviceStatus(string memory _device) public {
        // require(deviceNames[_device] == msg.sender, "Only the owner can change the device status");
    ...
    function recordDeviceData(uint256 _totalData, string memory _device) public {
        // require(deviceNames[_device] == msg.sender, "Only the owner can update the data");
    ...
    function getDeviceData(string memory _device) public view returns (string memory, uint256, uint256, address, bool) {
        DeviceData memory data = deviceRecords[_device];
        require(data.owner != address(0), "Device does not exist");

        return (data.device, data.totalData, data.timestamp, data.owner, data.enabled);
    }
}
